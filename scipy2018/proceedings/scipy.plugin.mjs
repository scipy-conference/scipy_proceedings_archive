    ...
    return {
      ...
      node: {
        ...
        data: {
          json: {
            scipy_scholarship,
            diversity_scholarship: diversity_scholarship && diversity_scholarship.length > 0 ? diversity_scholarship : void 0
          }
        }
      }
    };
...
var scipySlides = {
  ...
  run(data, vfile, opts) {
    const { json } = data.node.data;
    const { slides, posters, tools, lightning } = json;
    const nodes = [];
    if (slides) {
      nodes.push(
        u3("block", [
          u3("heading", { depth: 2 }, [u3("text", "Accepted Paper Slides")]),
          ...slides.map(ensureAuthors).map((item) => createSlidesCardDirective(item, opts.parseMyst))
        ])
      );
    }
    if (posters) {
...
var DoiBatchReader = class {
  ...
  getConferencePaper(entry) {
    ...
    const pages = select("[name=pages]", entry) ? [
      toText(select("[name=first_page]", entry)),
      toText(select("[name=last_page]", entry))
    ] : null;
    return {
      type: entry.name,
      title,
...
var scipyArticles = {
  ...
  run(data, vfile, opts) {
    ...
    return [
      ...entries.map(
        (entry) => createConferencePaperCardDirective(
          entry,
          opts.parseMyst,
...
export {
  scipy_plugin_default as default
};
