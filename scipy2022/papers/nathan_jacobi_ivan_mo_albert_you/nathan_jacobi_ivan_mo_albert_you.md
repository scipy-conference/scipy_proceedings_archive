...
The corpus within each bucket is represented by $D_{t}$, with *t* representing the temporal slice. Within each 2 month period, tweets were split into 12 pre-processed output csv files.
...
where *w* and *c* are two words in *V*. Embeddings generated by word2vec can be approximated by PMI matrices, where given embedding vectors utilize the following equation {cite:p}`dwe18`:
