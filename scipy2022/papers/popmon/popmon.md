...
[^footnote-1]: See <https://github.com/ing-bank/popmon> for code, documentation,
...
[^footnote-2]: Alternatively one may apply the Bonferroni correction to counteract
...
*LED*: Pearson correlation compared with previous histogram. The shifting points are correctly identified at every 5th of the LED dataset. Similar patterns are visible for other comparisons, e.g. $\chi^2$.
:::
...
*Hyperplane*: The incremental drift compared to the reference dataset is observed for the PhiK correlation between the predictions and the label.
:::
